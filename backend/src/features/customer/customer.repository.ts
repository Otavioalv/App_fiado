...
export class CustomerRepository {
    public async register(datasRegister: clienteInterface): Promise<clienteInterface>{
        ...
        try {
            ...
            const SQL = `
                ...
                VALUES ($1, $2, $3, $4) 
                RETURNING nome, senha, apelido, telefone;
            `
            const {nome, senha, apelido, telefone} = datasRegister;
            const values = [nome, senha, apelido, telefone];

            await client.query('BEGIN');
            const rows = (await client.query<clienteInterface>(SQL, values)).rows;
