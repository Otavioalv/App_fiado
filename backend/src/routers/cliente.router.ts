...
export const clienteRouter = async (router: FastifyInstance, options: FastifyPluginOptions) => {
    ...
    router.post("/product/list/:typeList", authorizedOptions("cliente"), async(req: FastifyRequest, res: FastifyReply) => {
        return await produtoController.listProductsByIdFornecedor(req, res);
    }); 

    router.post('/product/buy/list/:typeList/:toUser?', authorizedOptions("cliente"), async(req: FastifyRequest, res: FastifyReply) => {
